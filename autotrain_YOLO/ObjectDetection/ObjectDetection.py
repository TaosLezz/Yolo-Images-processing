import cv2
import os
from ObjectDetection.utils import draw_detections
...
class Yolo_V8_Detection:
    def __init__(self, pathWeight='', method=2, conf_thres=0.7, iou_thres=0.5):
        ...
        if method == 2:
            from ObjectDetection.Yolo_v8_onnxruntime import YOLOv8_onnxruntime
    ...
    def Predict(self, input_image):
        ...
        if self.method == 1:
            results = self.net.predict(imProcess, conf=self.conf, iou=self.iou)
...
class Yolo_V5_Detection:
    def __init__(self, pathWeight='', method=2, conf_thres=0.7, iou_thres=0.5):
        ...
        if method == 2:
            from ObjectDetection.Yolo_V5_onnx import YOLOv5_onnx
